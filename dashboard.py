...
if not df.empty:
    ...
    # 뿌연 투명도 및 반투명 스타일 제거 + 명확한 선명도 설정
    fig.update_traces(
        texttemplate=(
            "<b>%{label}</b><br>"
        ...
        # 선명도 강화용 속성 추가
        marker=dict(
            line=dict(width=1.5, color="#121212"),  # 각 블록 테두리를 또렷하게 분리
            pad=dict(t=25, l=4, r=4, b=4)            # 텍스트 여백을 확보해 겹침 방지
        ),
    ...
    fig.update_layout(
        ...
        paper_bgcolor="#121212",  # 배경 흐림 제거용 깔끔한 어두운 바탕
        plot_bgcolor="#121212",
    )
